...
func (b *Bot) callback(ctx context.Context, q *tgbotapi.CallbackQuery, updateID int64) error {
	...
	/*
		...
		case data == "v1:cancel":
			_ = b.db.ClearState(ctx, b.userID)
			return b.send("Draft dibatalkan.", menu())
		case data == "v1:newproject":
			return b.startNestedProject(ctx)
		case data == "v1:skipdesc":
			_, _, raw, nonce, ver, _, _ := b.db.GetState(ctx, b.userID)

			var d map[string]string
			_ = json.Unmarshal([]byte(raw), &d)
			d["description"] = ""
			return b.projectPreview(ctx, d, nonce, ver+1)
		case data == "v1:skipnotes":
			_, _, raw, nonce, ver, _, _ := b.db.GetState(ctx, b.userID)
			...
			d["notes"] = ""
			return b.backlogPreview(ctx, d, nonce, ver+1)
		case data == "v1:saveproject":
			return b.saveProject(ctx, updateID)
		case data == "v1:savebacklog":
			return b.saveBacklog(ctx, updateID)
		case strings.HasPrefix(data, "v1:archive:"):
			return b.archive(ctx, strings.TrimPrefix(data, "v1:archive:"), updateID)
		case strings.HasPrefix(data, "v1:restore:"):
			return b.restore(ctx, strings.TrimPrefix(data, "v1:restore:"), updateID)

		case data == "v1:focus":
			return b.focus(ctx)
		case data == "v1:projects":
			return b.projects(ctx)
		case data == "v1:list":
			return b.list(ctx)
		case data == "v1:add":
		...
		case data == "v1:help":
			return b.send("Pilih project aktif, quadrant, dan deadline wajib untuk setiap backlog.", menu())
		}
		if strings.HasPrefix(data, "v1:project:") {
			return b.callbackProject(ctx, strings.TrimPrefix(data, "v1:project:"))
		}
		if strings.HasPrefix(data, "v1:q:") {
			return b.callbackQuadrant(ctx, domain.Quadrant(strings.TrimPrefix(data, "v1:q:")))
		}
		if strings.HasPrefix(data, "v1:date:") {
			return b.callbackDate(ctx, strings.TrimPrefix(data, "v1:date:"))
		}
		if strings.HasPrefix(data, "v1:done:") {
			return b.complete(ctx, strings.TrimPrefix(data, "v1:done:"), updateID)
		}
...
func (b *Bot) callbackQuadrant(ctx context.Context, q domain.Quadrant) error {
	...
	_, _, raw, nonce, ver, _, err := b.db.GetState(ctx, b.userID)
	if err != nil {
		return err
	}
	var d map[string]string
	_ = json.Unmarshal([]byte(raw), &d)
	d["quadrant"] = string(q)
	return b.askDeadline(ctx, d, nonce, ver+1)
}
func (b *Bot) callbackDate(ctx context.Context, value string) error {
	_, _, raw, nonce, ver, _, err := b.db.GetState(ctx, b.userID)
	if err != nil {
		return err
	}
	...
	case "custom":
		return b.saveDraft(ctx, "backlog", "date", d, nonce, ver+1, "Masukkan tanggal YYYY-MM-DD atau DD-MM-YYYY.", cancelKeys())
	default:
		return nil
	}
	return b.askNotes(ctx, d, nonce, ver+1)
}
func (b *Bot) saveProject(ctx context.Context, updateID int64) error {
	flow, _, raw, nonce, _, _, err := b.db.GetState(ctx, b.userID)
	if err != nil {
		return err
	}
	if flow != "project" {
		return b.send("Draft project tidak ditemukan.", menu())
	}
	var d map[string]string
	_ = json.Unmarshal([]byte(raw), &d)
	now := time.Now()
	p, _, err := b.projectSvc.CreateWithMutationAndClearState(ctx, updateID, nonce, d["name"], d["description"], now, b.userID)
	if err != nil {
		return b.send("Project gagal disimpan; nama mungkin sudah digunakan.", menu())
	}
	if d["return_flow"] == "backlog" {
		returnNonce := d["return_nonce"]
		delete(d, "return_flow")
		delete(d, "return_step")
		delete(d, "return_nonce")
		d["project_id"] = p.ID
		return b.saveDraft(ctx, "backlog", "quadrant", d, returnNonce, 1, "Pilih prioritas Eisenhower.", [][]tgbotapi.InlineKeyboardButton{{tgbotapi.NewInlineKeyboardButtonData("Q1", "v2:quadrant:q1:"+returnNonce)}, {tgbotapi.NewInlineKeyboardButtonData("Q2", "v2:quadrant:q2:"+returnNonce)}, {tgbotapi.NewInlineKeyboardButtonData("Q3", "v2:quadrant:q3:"+returnNonce)}, {tgbotapi.NewInlineKeyboardButtonData("Q4", "v2:quadrant:q4:"+returnNonce)}, cancelKeys()[0]})
	}
	_ = b.db.ClearState(ctx, b.userID)
	return b.send("Project disimpan.", menu())

}
func (b *Bot) saveBacklog(ctx context.Context, updateID int64) error {
	flow, _, raw, nonce, _, _, err := b.db.GetState(ctx, b.userID)
	if err != nil {
		return err
	}
	if flow != "backlog" {
		return b.send("Draft backlog tidak ditemukan.", menu())
	}
	var d map[string]string
	_ = json.Unmarshal([]byte(raw), &d)
	p, err := b.db.GetProject(ctx, d["project_id"])
	if err != nil || p.Status != domain.ProjectActive {
		return b.send("Project tidak aktif.", menu())
	}
	now := time.Now()
	_, _, err = b.backlog.CreateWithMutationAndClearState(ctx, updateID, nonce, p.ID, d["title"], d["notes"], domain.Quadrant(d["quadrant"]), d["deadline"], now, b.userID)
	if err != nil {
		return err
	}
	_ = b.db.ClearState(ctx, b.userID)
...
func (b *Bot) archive(ctx context.Context, id string, updateID int64) error {
	_, err := b.projectSvc.ArchiveWithMutation(ctx, updateID, "archive:"+id, id, time.Now())
	if err != nil {
		return err
	}
	return b.send("Project diarsipkan.", menu())
}
func (b *Bot) restore(ctx context.Context, id string, updateID int64) error {
	_, err := b.projectSvc.RestoreWithMutation(ctx, updateID, "restore:"+id, id, time.Now())
	if err != nil {
		return err
	}
	return b.send("Project dipulihkan.", menu())
}
func (b *Bot) complete(ctx context.Context, id string, updateID int64) error {
	_, err := b.backlog.CompleteWithMutation(ctx, updateID, "complete:"+id, id, time.Now())
	if err != nil {
		return err
	}
	return b.send("Backlog ditandai selesai.", menu())
}
func (b *Bot) projects(ctx context.Context) error {
	ps, err := b.db.ListProjects(ctx, false)
	if err != nil {
		return err
	}
	if len(ps) == 0 {
		return b.send("Belum ada project.", [][]tgbotapi.InlineKeyboardButton{{tgbotapi.NewInlineKeyboardButtonData("Buat Project", "v1:newproject")}, {tgbotapi.NewInlineKeyboardButtonData("Kembali", "v1:cancel")}})
	}
	var s strings.Builder
	s.WriteString("<b>Projects</b>\n")
	for _, p := range ps {
		s.WriteString("• " + Escape(p.Name) + "\n")
	}
	return b.send(s.String(), [][]tgbotapi.InlineKeyboardButton{{tgbotapi.NewInlineKeyboardButtonData("Buat Project", "v1:newproject")}, menu()[0]})
}
